    ...
    public function parse($phpExel, Organisation $org)
    {
        ...
        foreach ($sheetData as $rowName => $rowValue) {
            ...
            if ($rowName >= 4 && $rowValue["B"] != "") {
                ...
                if ($rowValue["C"] != "") {
                    $user->setGivenName($rowValue["C"]);
                } else {
                    $user->setGivenName($rowValue["B"]);
                }
                if ($rowValue["K"] != "") {
                    $user->street = $rowValue["K"];
                }
                if ($rowValue["L"] != "") {
                    $user->postalCode = $rowValue["L"];
                }
                if ($rowValue["M"] != "") {
                    $user->l = $rowValue["M"];
                }
                if ($rowValue["N"] != "") {
                    $user->telephoneNumber = $rowValue["N"];
                }
                if ($rowValue["O"] != "") {
                    $user->mobile = $rowValue["O"];
                }
